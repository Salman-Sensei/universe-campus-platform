...
export function PopularUsers() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      follows?.forEach((f) => { followersMap[f.following_id] = (followersMap[f.following_id] || 0) + 1; });

      const enriched: PopularUser[] = profiles
        .filter((p) => p.user_id !== user?.id)
        .map((p) => ({ ...p, followers_count: followersMap[p.user_id] || 0 }));
        ...
        {users.map((u, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-center gap-3 group rounded-xl p-2 -mx-1 hover:bg-surface-hover transition-all duration-200"
            >
              <Link to={`/user/${u.username || u.user_id}`} className="flex items-center gap-3 flex-1 min-w-0">
                <Avatar className="h-9 w-9 ring-2 ring-border/30 group-hover:ring-primary/30 transition-all">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors truncate">{name}</p>
                  <p className="text-[11px] text-muted-foreground">{u.followers_count} follower{u.followers_count !== 1 ? "s" : ""}</p>
                </div>
              </Link>
              {user && (
                <Button
